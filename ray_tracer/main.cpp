...
void simple_world() {
    ...
    auto material_center = std::make_shared<Lambertian>(Color(1.0, 0.4, 0.4));
    auto material_right = std::make_shared<Lambertian>(perlin_texture);
    auto material_front = std::make_shared<Dielectric>(1.5);
    auto material_left_front = std::make_shared<Lambertian>(Color(0.55, 0.78, 1.0));
    ...
    world.add(std::make_shared<Sphere>(centre_front, centre_front, 0.15, material_front));
    world.add(std::make_shared<Sphere>(centre_left_front, centre_left_front + Vec3(0, 0.12, 0), 0.15, material_left_front));

    world = HittableList(std::make_shared<BVHNode>(world));

    // Camera
    Camera camera{ 16.0/9.0, 1000, 100, 50, 20, Point3(0, 0, 6), Point3(0, 0, 0), Vec3(0, 1, 0), 0.05, 6 };
    camera.render(world);
}
